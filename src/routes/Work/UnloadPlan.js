...
@Form.create()
class UnloadPlan extends React.Component {
  constructor(props) {
    super(props);
    this.state = { 
      editingKey: '',
    ...
    this.columns = [
        ...
        },
        {
            ...
            render:(text,record)=>(
              <a onClick={()=>this.ToUnload(record)}>{record.shipName}</a>
            ),
        },  
        {
            title:'货名',
  ...
  onCreateUnloadPlan = (params) => {
	const { dispatch } = this.props;
	dispatch({type: 'unloadplan/createUnloadPlan', payload: params});
  };
  onDeleteUnloadPlan = (id) => {
	const { dispatch } = this.props;
	dispatch({type: 'unloadplan/deleteUnloadPlan', payload: {id}});
  };
  onUpdateUnloadPlan = (params) => {
	const { dispatch } = this.props;
	dispatch({type: 'unloadplan/updateUnloadPlan', payload: params});
  }; 
  onListUnloadPlanByPage = (params) => {
	const { dispatch } = this.props;
	dispatch({type: 'unloadplan/listUnloadPlanByPage', payload: params});
  };

  onListShip = () => {
	const { dispatch } = this.props;
	dispatch({type: 'unloadplan/listShip'});
  };
  onListCargo = (params) => {
	const { dispatch } = this.props;
	dispatch({type: 'unloadplan/listCargo'});
  };
  ...
  ToUnload=(record)=>{
    ...
    dispatch(routerRedux.push(`/work/unloadmanage?key=${record.id}`));
  }

  componentDidMount(){
      const params = {pageNo: 1};
      this.onListUnloadPlanByPage(params);
      this.onListShip();
      this.onListCargo();
  }
  ...
  handleAdd = fields => {
    let listShipId = [];
    this.props.unloadplan.listShip.map((item)=> listShipId.push(item.id));
    let listCargoId = [];
    this.props.unloadplan.listCargo.map((item)=> listCargoId.push(item.id));
    const params = {
      beginDate: fields.beginDate.format('YYYY-MM-DD'),
      shipId: fields.shipId,
      shipName: this.props.unloadplan.listShip[listShipId.indexOf(fields.shipId.toString())].cname,
      cargoId: fields.cargoId,
      cargoName: this.props.unloadplan.listCargo[listCargoId.indexOf(fields.cargoId.toString())].cname,
      totalWeight: fields.totalWeight,

    };
    this.onCreateUnloadPlan(params);
  ...
  handleUpdate(form, id) {
    form.validateFields((error, row) => {
      ...
      const params = {
        ...
        shipId: row.shipId,
        shipName: this.props.unloadplan.listShip[listShipId.indexOf(row.shipId.toString())].cname,
        cargoId: row.cargoId,
        cargoName: this.props.unloadplan.listCargo[listCargoId.indexOf(row.cargoId.toString())].cname,
        totalWeight: row.totalWeight,
      };
      this.onUpdateUnloadPlan(params);
  ...
  // 根据分页查询
  getUnloadPlanListByPage = (pageNumber)  => {
  ...
  handleList = (pageNo) => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      this.onListUnloadPlanByPage(values);
    });
  }
    ...
    const columns = this.columns.map((col) => {
      ...
      return {
        ...
        onCell: record => ({
          ...
          listship: this.props.unloadplan.listShip,    
          listcargo: this.props.unloadplan.listCargo, 
        }),
      };
    });
    ...
    return (
      <PageheaderLayout>
        ...
        <Table
          ...
          bordered
          dataSource={this.props.unloadplan.items}
          columns={columns}
        ...
        />
        <Pagination
            current={Number(this.props.unloadplan.pageNo)}
            total={Number(this.props.unloadplan.total)}
            onChange={this.getUnloadPlanListByPage}
            style={{float: 'right', marginTop: '20px'}}
          />
        <CreateForm {...parentMethods} modalVisible={modalVisible} listShip={this.props.unloadplan.listShip} listCargo={this.props.unloadplan.listCargo} />
        </Card>
      </PageheaderLayout>
      );
    }
...
export default connect(({ unloadplan }) => ({unloadplan}))(UnloadPlan);